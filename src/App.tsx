...
const getBookmarkCode = (): string => {
  ...
  code += "function _ttE(){localStorage.setItem('tt_lobby_enabled','1');localStorage.setItem('tt_lobby_id','0');localStorage.setItem('tt_lobby_host',L0);location.reload();}";
  code += "function _ttD(){localStorage.removeItem('tt_lobby_enabled');localStorage.removeItem('tt_lobby_id');localStorage.removeItem('tt_lobby_host');if(window._ttOrigWS){window.WebSocket=window._ttOrigWS;}window._ttHook=0;location.reload();}";
  // Auto-inject hook on page load
  ...
  code += "sw.onclick=function(){if(on){_ttD();}else{_ttE();}};";
  ...
  code += "tip.textContent=on?'页面已刷新，点击多人游戏进入 Lobby 0':'点击开启后页面将刷新';";
